...
public class CandidateVersionBusinessProcessControllerTest {
  ...
  private static final String BUSINESS_PROCESS_CONTENT = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
      ...
      "  <bpmn:process id=\"name\" name=\"title\" isExecutable=\"true\">\n" +
  ...
  public void getBusinessProcess() {
    Mockito.when(businessProcessService.getProcessContent("businessProcess", CANDIDATE_VERSION_ID))
        .thenReturn(BUSINESS_PROCESS_CONTENT);

    mockMvc.perform(get(BASE_REQUEST + "/businessProcess"))
        .andExpectAll(
            status().isOk(),
  ...
  public void createBusinessProcess() {
    mockMvc.perform(MockMvcRequestBuilders.post(
            BASE_REQUEST + "/{businessProcessName}",  "name")
    ...
    Mockito.verify(businessProcessService).createProcess("name", BUSINESS_PROCESS_CONTENT, CANDIDATE_VERSION_ID);
  }
  ...
  public void updateBusinessProcess() {
    ...
    Mockito.verify(businessProcessService).updateProcess(BUSINESS_PROCESS_CONTENT, "name", CANDIDATE_VERSION_ID);
  ...
  public void deleteBusinessProcess() {
    ...
    Mockito.verify(businessProcessService).deleteProcess("name", CANDIDATE_VERSION_ID);
  }
}
