...
public class JGitWrapperMock {
  ...
  public void mockCloneCommand(String versionName) {
    if (!Objects.equals(versionName, gerritPropertiesConfig.getHeadBranch())) {
    ...
    Mockito.when(cloneCommand.setCredentialsProvider(Mockito.refEq(credentialsProvider))).thenReturn(cloneCommand);
    ...
    Mockito.when(loader.getCachedBytes()).thenReturn("This is our response".getBytes(StandardCharsets.UTF_8));
  ...
  public void mockPullCommand() {
    ...
    var credentialsProvider = new UsernamePasswordCredentialsProvider(
        gerritPropertiesConfig.getUser(), gerritPropertiesConfig.getPassword());
    Mockito.when(pullCommand.setCredentialsProvider(Mockito.refEq(credentialsProvider))).thenReturn(pullCommand);
  ...
  public void mockFetchCommand(ChangeInfoDto changeInfoDto) {
    ...
    Mockito.when(fetchCommand.setRefSpecs(Mockito.refEq(changeInfoDto.getRefs()))).thenReturn(fetchCommand);
  ...
  public void mockCheckoutCommand() {
    ...
    Mockito.when(checkoutCommand.setName("refs/heads/" + gerritPropertiesConfig.getHeadBranch())).thenReturn(checkoutCommand);
  ...
  public void mockGetForm(FormDetailsShort formDetails) {
    ...
    Mockito.when(loader.getBytes()).thenReturn((gson.toJson(formDetails).getBytes(StandardCharsets.UTF_8)));
  }

  @SneakyThrows
  public void mockGetFormsList(List<FormDetailsShort> list) {
    Mockito.when(jGitWrapper.getTreeWalk(eq(repository), anyString(), eq(revTree))).thenReturn(treeWalk);
