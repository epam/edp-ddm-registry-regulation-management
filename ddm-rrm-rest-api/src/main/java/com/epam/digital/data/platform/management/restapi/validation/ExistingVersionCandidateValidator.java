...
/**
 * Validates if requested version candidate exists
 */
@RequiredArgsConstructor
@Slf4j
public class ExistingVersionCandidateValidator implements
    ConstraintValidator<ExistingVersionCandidate, Integer> {
  ...
  public boolean isValid(Integer value, ConstraintValidatorContext context) {
    try {
      var ignoredResult = versionManagementService.getVersionDetails(String.valueOf(value));
    } catch (GerritChangeNotFoundException e) {
      log.info("Requested version candidate {} does not exist", value);
      return false;
    }
    return true;
  }
