...
image::rest-api-structure.svg[admin-portal-backend RestAPI structure]
...
[NOTE]
Client is not restricted to not use ETag's. In this case optimistic locking mechanism won't be used and owerwrites could happens for a resources. More details could be found xref:admin-portal-backend:rest-api/rest-api-partials/optimistic-locking.adoc [here].
