...
@DisplayName("JGitService#deleteAndSubmit")
class JGitServiceDeleteAndSubmitTest extends AbstractJGitServiceTest {
  ...
  static final String COMMIT_MESSAGE = "deleted file ";
  ...
  void setUp() {
    ...
    Assertions.assertThat(new File(repoDir, FILE_PATH).createNewFile()).isTrue();

    Mockito.doReturn(git).when(jGitWrapper).open(repoDir);
    Mockito.doReturn("content").when(jGitWrapper).readFileContent(
        new File(repoDir, FILE_PATH).toPath());
  ...
  @Test
  @DisplayName("should 'git rm' file, commit, add remote and push with submit parameter")
  @SneakyThrows
  void shouldDeleteAndPushFile() {
    jGitService.deleteAndSubmit(REPO_NAME, FILE_PATH,
        String.format("\"%s\"", "content" .hashCode()));
  ...
  @Test
  @DisplayName("should 'git rm' file, commit, add remote and push and get merge conflict")
  @SneakyThrows
  void shouldThrowMergeConflictException() {
    Mockito.doReturn(fetchCommand).when(git).fetch();
    Mockito.doReturn(fetchCommand)
        .when(fetchCommand).setCredentialsProvider(
            Mockito.refEq(new UsernamePasswordCredentialsProvider(USER, PASS)));
    ...
    Mockito.doReturn(resetCommand)
        .when(resetCommand).setRef(Constants.DEFAULT_REMOTE_NAME + "/" + HEAD_BRANCH);

    var pushResult = Mockito.mock(PushResult.class);
    var update = Mockito.mock(RemoteRefUpdate.class);
    Mockito.doReturn(RemoteRefUpdate.Status.REJECTED_NONFASTFORWARD).when(update).getStatus();
    Mockito.doReturn(List.of(update)).when(pushResult).getRemoteUpdates();

    Mockito.doReturn(List.of(pushResult)).when(pushCommand).call();

    var eTag = String.format("\"%s\"", "content" .hashCode());
    assertThrows(MergeConflictException.class,
        () -> jGitService.deleteAndSubmit(
            REPO_NAME, FILE_PATH, eTag));
